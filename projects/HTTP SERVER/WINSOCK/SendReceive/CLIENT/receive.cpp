...
//CLIENT SIDE. RECEIVING END
/* 
int recv(
    SOCKET s,        // the socket you're receiving from
    char *buf,       // buffer to store data
    int len,         // size of the buffer
    int flags        // usually 0
);
Returns number of bytes received, 0 if connection closed, or SOCKET_ERROR if it fails.
You have to give it a buffer to store the message.
*/
//CLIENT SIDE
int main(){
